...
export const ftSnapshot = async (pubKey) => {
    let numParitions = 2;
        ...
        let promise = new Promise(async (resolve, reject) => {
            ...
            while (true) {
                ...
                if (result.token_accounts.length == 0) {
                    ...
                    current = result.token_accounts[result.token_accounts.length - 1].address;
    ...
    total = total.map((item)=>{ return {owner:item.owner, balance:item.amount}})
    console.log(total);
    return total
};
