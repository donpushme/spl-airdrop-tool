...
export const ftSnapshot = async (pubKey, min, max) => {
  let numParitions = 5; //This should be changee to get the list faster
    ...
    let promise = new Promise(async (resolve, reject) => {
      ...
      while (true) {
        ...
        if (result.token_accounts.length == 0) {
          console.log(`Ended in partision${i}`)
          break;
        } else {
          current =
  ...
  total = total.map((item) => {
    if(blacklistedMarketplaceOwners.indexOf(item,owner) < 0) return { owner: item.owner, balance: item.amount };
  });
  // console.log(total);
  if (max != 0) {
