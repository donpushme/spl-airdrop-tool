...
export function makeURLwithMultiplier(path, counts, multiplier, totalAmount) {
  ...
  const stringArray = counts.map((count, index) => count.join("_"));
  let head = path;
...
export function getParams(path) {
  ...
  if (path.indexOf("&counts=") > 0) {
    ...
    counts = countsString.split("-");
    counts = counts.map((count) => { count.split("_") });
  ...
  return { flag, fileId, address, counts, multiplier, totalAmount };
}
