...
import { TOKEN_PROGRAM_ID, splToken } from "@solana/spl-token";
...
export const fetchWalletTokens = async (wallet) => {
  ...
  try {
    ...
    for (const cur of accounts) {
      ...
      try {
        const metadata = await getToken(mint);
        if(metadata.interface !== "FungibleToken") continue
...
export const checkBalance = async (publicKey, mint, SolAmount, tokenAmount) => {
  ...
  if(solBalance < SolAmount) return false;
  const tokenBalance = await getTokenBalance(publicKey, mint);
  console.log(tokenBalance, "TOKENBALANCE")
  if(tokenBalance < tokenAmount) return false;
  return true;
}
