...
import { RPC_URL, HELIUS_RPC } from "@/config";
import {
  METADATA_PROGRAM,
  SYSTEM_PROGRAM,
  TOKEN2022_PROGRAM,
...
export const inputRouter = async (pubKey, listType) => {
  ...
  switch (owner) {
    ...
    case TOKEN2022_PROGRAM:
      ...
      const { tokenStandard, data } = metadata.data;
      if (tokenStandard == 4) {
 ...
 * @returns
 */
const getAccountInformation = async (pubKey) => {
