...
export const getToken = async (mint) => {
  try {
    ...
    const price  = typeof tokenData?.token_info?.price_info?.price_per_token == "undefined" ? 0 : tokenData?.token_info?.price_info?.price_per_token;
    const token = {
      interface: tokenData.interface,
      address: mint,
      decimal: tokenData.token_info.decimals,
      price,
